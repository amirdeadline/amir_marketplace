...
1. Nearest ancestor with `.amir/project.yaml`; if none â†’ STOP: "Not an Amir project â€” nothing to
   remove."
2. Engine:
   ```powershell
   python "$env:USERPROFILE\.amir\bin\amirctl.py" remove-project-config
   ```
   If `amirctl.py` is missing â†’ report it honestly; a supervised manual removal is possible but
   riskier â€” proceed only with the full plan/confirm flow below and recommend re-provisioning
   the engine.

## Procedure (destructive-action rule â€” strict order)
...
2. **Show the deletion plan**: every file/directory to be removed, identified as Amir-managed by
   ...
   - unrelated Cursor config (user's own `.cursor/rules`, other commands, unrelated
     `.cursor/mcp.json` entries â€” merge-remove only Amir entries, never the whole file)
...
4. Execute; deregister the project from `%USERPROFILE%\.amir\registry\projects.yaml`.
5. **Never uninstall Graphify globally**, never touch the graphify CLI, never remove
   user-scope plugins â€” this command's scope ends at the project boundary.
